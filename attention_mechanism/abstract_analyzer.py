...
# Load SciBERT model and tokenizer outside for efficiency
@st.cache_resource
def load_scibert():
...
st.header("Implementation with SciBERT")
st.markdown("""
Yes, it is possible to implement the higher-order attention mechanism using SciBERT as the base encoder. SciBERT is a pre-trained BERT model fine-tuned on scientific literature, making it ideal for domains like materials science and ferroelectricity. The original pseudo-code in the scratch idea already suggests using SciBERT for encoding.

To extend this, higher-order attention can be added on top of SciBERT's embeddings by:
- Extracting entity spans using a fine-tuned NER layer.
- Applying a custom higher-order attention module (e.g., via tensor factorizations or multi-hop attention) to score relationships.

In practice, libraries like Hugging Face Transformers allow loading SciBERT and modifying its architecture or adding post-processing layers for higher-order reasoning. Research papers (e.g., on multi-level feature fusion and attention enhancements for NER) demonstrate similar integrations in scientific NLP tasks.

Below is a demo where you can input an abstract (truncated to 512 tokens). The app will:
- Reject out-of-scope abstracts (not related to ferroelectricity).
- For in-scope abstracts, use SciBERT to compute embeddings and attention weights.
- Visualize pairwise attention (from SciBERT's last layer) to show how concepts are 'attentively grasped'.
- Extract key ferroelectric concepts and quantitative values (using simple keyword and regex for demo; in full impl, use fine-tuned NER).
This demonstrates attentive grasping by showing high attention scores between related tokens (e.g., material, property, value).
""")

st.subheader("Demo: Processing Ferroelectricity Abstract")
abstract = st.text_area("Enter an abstract related to ferroelectricity (up to 512 tokens)", height=200)
process_abstract_button = st.button("Process Abstract")
...
if process_abstract_button and abstract:
    ...
    if not any(kw.lower() in abstract.lower() for kw in ferro_keywords):
        st.warning("Abstract appears out-of-scope for ferroelectricity (no relevant keywords found). Rejected.")
    else:
        st.success("Abstract accepted as in-scope. Processing with SciBERT...")

        # Tokenize and truncate to 512
        inputs = tokenizer(abstract, return_tensors="pt", truncation=True, max_length=512, padding=True)

        # Get outputs
        outputs = model(**inputs)
        embeddings = outputs.last_hidden_state
        attentions = outputs.attentions  # tuple of attentions

        # Last layer attention, average over heads
        last_attn = attentions[-1][0]  # [heads, seq_len, seq_len]
        avg_attn = last_attn.mean(dim=0).detach().numpy()

        # Tokens
        tokens = tokenizer.convert_ids_to_tokens(inputs['input_ids'][0])

        # Visualize attention
        st.subheader("Attention Map (Showing How Concepts Are Attentively Grasped)")
        st.markdown("The heatmap shows pairwise attention weights from SciBERT's last layer. High values indicate strong relational focus, e.g., between quantitative values and properties.")
        fig, ax = plt.subplots(figsize=(12, 10))
        sns.heatmap(avg_attn, xticklabels=tokens, yticklabels=tokens, cmap="YlGnBu", ax=ax)
        st.pyplot(fig)

        # Extract grasped concepts
        ...
        # Extract quantitative (simple regex)
        st.subheader("Extracted Quantitative Concepts")
        # Regex for number + unit (common in ferroelectrics)
        ...
        # Simulate rejection of out-of-scope terms (for demo, assume all terms in accepted abstract are in-scope)
